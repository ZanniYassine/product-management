...
import {isCategoriesValid, isDateValid, isNameValid} from './validators';
...
const ProductForm = (props) => {
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        if (!isNameValid(name) || !isCategoriesValid(categories) || !isDateValid(new Date(expirationDate))) return;
        props.onSave({
            name,
            brand,
            rating,
        ...
        <Form onSubmit={onSubmit}>
            ...
            <FormGroup>
                ...
                <Input
                    ...
                    value={rating}
                    onChange={({target}) => {
                        if (target.value >= 8) {setFeatured(true)} else {setFeatured(false)} 
            ...
            <FormGroup>
                ...
                <Input
                    invalid={!isDateValid(new Date(expirationDate))}
                    type="date"
                    name="expirationDate"
                    id="expirationDate"
